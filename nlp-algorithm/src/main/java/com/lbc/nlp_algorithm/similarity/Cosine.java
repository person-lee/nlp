...
public class Cosine extends AbstractSimilarity<String>{  
	
	private static Cosine cosine = null;
	...
	public double sim(String subStr, String obStr) {
		if (subStr != null && subStr.trim().length() > 0 && obStr != null  
                && obStr.trim().length() > 0) { 
			return countSim(getSimilarity(subStr, obStr));
		} else {  
    ...
    private Map<Integer, int[]> getSimilarity(String doc1, String doc2) {  
    ...
    private double countSim(Map<Integer, int[]> AlgorithmMap) {
    ...
    public static void main(String[] args) {  
		ISimilarity<String> similarity = Cosine.getInstance();
