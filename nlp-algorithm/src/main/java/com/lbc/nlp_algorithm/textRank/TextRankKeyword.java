package com.lbc.nlp_algorithm.textRank;
...
public class TextRankKeyword {
    ...
    public String getKeyword(String title, String content) {
        ...
        Map<String, Set<String>> words = Maps.newHashMap();
        Queue<String> que = Lists.newLinkedList();
        for (String w : wordList) {
            if (!words.containsKey(w)) {
                words.put(w, Sets.<String>newHashSet());
            }
            que.offer(w);
            if (que.size() > 5) {
                que.poll();
...
//        System.out.println(words);
        Map<String, Float> score = Maps.newHashMap();
        ...
        List<Map.Entry<String, Float>> entryList = Lists.newArrayList(score.entrySet());
...
//        System.out.println(entryList);
	...
	public boolean shouldInclude(Term term) {
		if (term.getNatureStr().startsWith("n")
			...
			if (StringUtils.isNotBlank(StringUtil.removeSpecialChars(term.getName()))) {
				return true;
			}
		}
